...
const INTRO_LINES = [
  "現在輪到它自己試試看了。",
  "它會照剛剛看過的例子，來分析這些食物是不是蘋果。",
];

type Phase = "intro" | "reveal" | "feedback";

export default function Screen3Test1({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    new Audio(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/Voice/sound effects/${item.round1Correct ? "correct" : "error"}.mp3`).play().catch(() => {});
  }, [showResult, item]);
        ...
        {phase !== "intro" && (
          <>
            <p className="text-sm tracking-wide" style={{ color: `${GREEN}88` }}>
              {phase === "reveal" && scanning && !showResult ? "食物分類機正在判斷⋯⋯" : phase === "reveal" && !scanning ? "準備分析食物" : "分類完成"}
            </p>
            <div style={{ padding: "8px 24px", border: `1px solid ${GREEN}44`,
              ...
              {phase === "reveal" && scanning && !showResult ? "正在判斷…" : phase === "reveal" && !scanning ? "待機中" : "判斷完成"}
            </div>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              {(showResult || phase === "feedback") && (
                <div style={{
                  ...
                  border: `2px solid ${item.round1Correct ? "#4ade80" : "#f87171"}`,
                  background: item.round1Correct ? "rgba(74,222,128,0.1)" : "rgba(248,113,113,0.1)",
                  color: item.round1Correct ? "#4ade80" : "#f87171",
                }}>
                  分析結果：{guessLabel(item.round1Guess)}
                </div>
              )}
            </div>
          </>
